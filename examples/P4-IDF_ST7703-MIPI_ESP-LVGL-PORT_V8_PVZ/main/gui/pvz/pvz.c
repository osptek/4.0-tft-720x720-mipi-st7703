...
#define MONITOR_HOR_RES  800 //监控水平_分辨率
#define  max_zb_count  20  //僵尸最大数量
#define  max_quantity  15  //每种植物最大数量
#define  zb_period     4000   //僵尸产生周期
#define  fast_zb_period  1000  //快速产生僵尸
#define  super_fast_zb_period 500  //超快速产生僵尸
#define  zidan_speed   7    //子弹每次飞5像素
#define  shine_period  7000  //阳光生产周期
#define  plant_stackable  0  //ֲ植物可叠加放置（1可叠加，0不可叠加）
#define  sunflower_add_sun_period  7000   //向日葵阳光生产周期ms
#define  add_zidan_period  1000  //上弹周期ms
#define  add_shandian_period  1000  //添加闪电时期
...
LV_IMG_DECLARE(shandian_img)
LV_IMG_DECLARE(shandian_btn_img)
...
static plant_type  shandian[max_quantity]={0,};
...
static void obj_set_y_anim(void * var, int32_t v);
...
static void shandian_creat_cb(card_btn_type * card_btn,int x,int y);
static void add_shandian_cb(lv_timer_t * t);
static void shandian_anim_cb(void * var, int32_t v);
static void shandian_delect_cb(lv_anim_t * a);
...
const static zb_class_type zb_class[]={ {Normalzb,(void*)&zb1,20,150,0,zb_dead_anim,Normalzb_start_anim_cb},
                                        ...
                                        {qiqiuzb,(void*)&qiqiu_zb_img,20,50,0,carzb_del_cb,qiqiuzb_start_anim_cb},
									  };
...
static card_btn_type card_btn[]={	{0,(void *)&sun_btn_img,sunflower_card,0,btn_cliked_cb,sunflower_price,sunflower_creat_cb},
                                    ...
                                    {0,(void *)&shandian_btn_img,shandian_card,0,btn_cliked_cb,shandian_price,shandian_creat_cb},
                                };
...
void pvz_start()
{
        ...
        lv_memset_00(shandian,sizeof( shandian));
  ...
  int i;
  	...
  	score_lable=lv_label_create(map1);
		...
		for(i=0;i<sizeof(card_btn)/sizeof(card_btn_type);i++)
		{
			...
			lv_obj_set_user_data(card_btn[i].card_btn,&card_btn[i]);
            card_btn[i].ispushed=0;

            lv_anim_t a;
            lv_anim_init(&a);
            lv_anim_set_var(&a,card_btn[i].card_btn);
            lv_anim_set_exec_cb(&a,obj_set_y_anim);
            lv_anim_set_time(&a,500);
            lv_anim_set_delay(&a,i*50);
            lv_anim_set_values(&a,-42,2);
            lv_anim_start(&a);
    ...
    for(i=0;i<5;i++)
        {
					...
					lv_obj_set_user_data(little_car[i].zidan,&little_car[i]);
...
static void wogua_dead_cb3_end(lv_anim_t * a)
{
    lv_obj_t * user=(lv_obj_t *)a->var;
    lv_img_set_src(user,&wogua_dead_img);
}
...
{
	int x1,y1,x2,y2,i,j;
	static lv_coord_t * p,*p1;
	//lv_mem_free(p);

	plant_type * shandian=(plant_type *)t->user_data;
	x1=lv_obj_get_x(shandian->plant)+30;
	y1=lv_obj_get_y(shandian->plant)+10;
    ...
    {
        j=rand()%max_zb_count;
        if(zb_matrix[j].blood>0)goto addshandian;
        ...
        if(zb_matrix[j].blood==0)
            {
              zb_matrix[j].zb_class->zb_del_cb(&zb_matrix[j]);
                ...
                x2=lv_obj_get_x(zb_matrix[j].zb)+30;
                y2=lv_obj_get_y(zb_matrix[j].zb)+20;

                int xcell=(x2-x1)/9;
                int ycell=(y2-y1)/9;

                p=(lv_coord_t *)lv_mem_alloc((sizeof (lv_point_t))*10);
                p[0]=x1;p[1]=y1;p[18]=x2;p[19]=y2;
                ...
                lv_obj_t * shandian_track1=lv_line_create(map1);
                lv_obj_set_style_line_width(shandian_track1,7,0);
                lv_obj_set_style_line_color(shandian_track1,lv_color_hex(0x0044ff),0);
                lv_obj_set_style_line_rounded(shandian_track1,true,0);
                lv_obj_move_foreground(shandian_track1);


                lv_obj_t * shandian_track2=lv_line_create(map1);
                lv_obj_set_style_line_width(shandian_track2,3,0);
                lv_obj_set_style_line_color(shandian_track2,lv_color_hex(0xffffff),0);
                lv_obj_set_style_line_rounded(shandian_track2,true,0);
                lv_obj_move_foreground(shandian_track2);

                lv_obj_set_user_data(shandian_track1,shandian_track2);
                lv_obj_set_user_data(shandian_track2,p);



                lv_anim_t a;
                lv_anim_init(&a);
                lv_anim_set_var(&a,shandian_track1);
                lv_anim_set_exec_cb(&a,shandian_anim_cb);
                lv_anim_set_time(&a,300);
                lv_anim_set_values(&a,2,7);

                lv_anim_set_ready_cb(&a, shandian_delect_cb);
                lv_anim_start(&a);
...
static void shandian_anim_cb(void * var, int32_t v)
{
     ...
     lv_coord_t * p;
     lv_obj_t * shandian_track1=(lv_obj_t *)var;
     lv_obj_t * shandian_track2=(lv_obj_t *)lv_obj_get_user_data(shandian_track1);
     p=(lv_coord_t *)lv_obj_get_user_data(shandian_track2);

     int xoffset=(p[18]-p[0])<0?(p[0]-p[18])/9:(p[18]-p[0])/9;
     int yoffset=(p[19]-p[1])<0?(p[1]-p[19])/9:(p[19]-p[1])/9;

    for(int i=2;i<18;i+=2)
    {
        p[i]=p[i]-yoffset/4+rand()%(yoffset+1)/2;
        p[i+1]=p[i+1]-xoffset/4+rand()%(xoffset+1)/2;

    }

     lv_line_set_points(shandian_track1,(const lv_point_t *)p,10);
     lv_line_set_points(shandian_track2,(const lv_point_t *)p,10);
}
...
{
     lv_coord_t * p;
     lv_obj_t * shandian_track1=(lv_obj_t *)a->var;
     lv_obj_t * shandian_track2=(lv_obj_t *)lv_obj_get_user_data(shandian_track1);
     p=(lv_coord_t *)lv_obj_get_user_data(shandian_track2);
     lv_mem_free(p);
     lv_obj_del(shandian_track2);
     lv_obj_del(shandian_track1);
...
static void dici_anim_cb(void * var,int v)
{
...
static void wogua_delect_zb_cb(lv_anim_t * a)
{
    ...
    lv_obj_t * user=(lv_obj_t *)a->var;
    lv_img_set_src(user,&wogua_dead_img);
    lv_obj_set_y(user,lv_obj_get_y(user)+27);
}
...
static void car_start_move(void * var, int32_t v)
{
    ...
    j=((zidan_type* )lv_obj_get_user_data(((lv_obj_t *)var)))->y;
	...
	if(chanzi_btn_select)
		{
			for(j=0;j<max_quantity;j++)
			{
                ...
                if(shandian[j].alive==1&&shandian[j].x==x&&shandian[j].y==y)
				{
					  map_flag[y][x]=0;
						shandian[j].alive=0;
						shandian[j].x=0;
						shandian[j].y=0;
						lv_anim_del(&shandian[j],plant_anim_cb);
						lv_obj_del(shandian[j].plant);
						lv_timer_del(shandian[j].timer);
						chanzi_btn_select=0;
						lv_obj_set_style_border_opa(chanzi_btn,0,LV_PART_MAIN);
						lv_obj_clear_flag(map1, LV_OBJ_FLAG_CLICKABLE);
						break;
...
static void shandian_creat_cb(card_btn_type * card_btn,int x,int y)
{
    ...
    for(j=0;j<max_quantity;j++)
				{
					if(shandian[j].alive==0)
							{
								shandian[j].alive=1;
								shandian[j].x=x;
								shandian[j].y=y;
								shandian[j].plant=lv_img_create(map1);
								lv_img_set_src(shandian[j].plant, &shandian_img);
								lv_obj_set_pos(shandian[j].plant,x*49+18,y*54+44);
								lv_img_set_pivot(shandian[j].plant, 27, 50);
								...
								shandian[j].timer=lv_timer_create(add_shandian_cb,add_shandian_period,&shandian[j]);
...
static void add_jiguang_cb(lv_timer_t * t)
{
        ...
        lv_obj_del_delayed(jiguang_track2,550);
 				lv_obj_set_user_data(jiguang_track2,jiguang_track1);
...
{
    lv_obj_t * xxx=(lv_obj_t *)var;
    lv_obj_t * yyy=lv_obj_get_user_data(xxx);
					...
					if(wogua[j].alive==0)
							{
								...
								lv_obj_set_user_data(wogua[j].plant,&wogua[j]);
					...
					if(dici[j].alive==0)
							{
								...
								lv_obj_set_user_data(dici[j].plant,&dici[j]);
...
void pvz_start()
{
		...
		screen=lv_tileview_create(lv_scr_act());
		lv_obj_set_size(screen,720,480);
		lv_obj_clear_flag(screen, LV_OBJ_FLAG_SCROLLABLE);
		map1=lv_img_create(screen);
		lv_obj_set_size(map1,720,480);
		lv_img_set_src(map1, &pvz_map_4);
		lv_obj_clear_flag(map1, LV_OBJ_FLAG_SCROLLABLE);
		lv_obj_add_event_cb(map1,map_click_cb,LV_EVENT_RELEASED,0);
...
static void game_start(lv_event_t * e)
{
		...
		lv_label_set_text_fmt(score_lable, "%d",sun_score);
		lv_obj_set_style_text_color(score_lable,lv_color_hex(0xffffff), LV_PART_MAIN);
		lv_obj_set_pos(score_lable,15,45);

		for(i=0;i<sizeof(card_btn)/sizeof(card_btn_type);i++)
		{
			card_btn[i].card_btn=lv_img_create(map1);
			lv_obj_set_pos(card_btn[i].card_btn,83+(i)*49,3);
			lv_img_set_src(card_btn[i].card_btn, card_btn[i].img_src);
			lv_img_set_pivot(card_btn[i].card_btn,0,0);
			lv_obj_set_style_img_recolor(card_btn[i].card_btn,lv_color_hex(0x000000),LV_PART_MAIN);
			lv_obj_set_style_img_recolor_opa(card_btn[i].card_btn,200,LV_PART_MAIN);
			lv_obj_set_style_outline_width(card_btn[i].card_btn, 0, LV_PART_MAIN);
			lv_obj_set_style_outline_color(card_btn[i].card_btn, lv_color_hex(0xffffff), LV_PART_MAIN);
		...
		botton_exit=lv_btn_create(map1);
		lv_obj_set_style_bg_color(botton_exit,lv_color_hex(0x6f3011), LV_PART_MAIN);
		lv_obj_set_pos(botton_exit,600,2);
		exit_lable=lv_label_create(botton_exit);
		lv_label_set_text(exit_lable, "<EXIT");
		lv_obj_set_style_text_color(exit_lable,lv_color_hex(0xffffff), LV_PART_MAIN);
		lv_obj_add_event_cb(botton_exit,exit_game_cb,LV_EVENT_RELEASED,0);
		...
		chanzi_btn=lv_btn_create(map1);
		lv_obj_set_pos(chanzi_btn,525,-1);
		lv_obj_set_size(chanzi_btn,65,65);
		lv_obj_set_style_shadow_opa(chanzi_btn,0,LV_PART_MAIN);
		lv_obj_set_style_bg_opa(chanzi_btn,0,LV_PART_MAIN);
		lv_obj_set_style_border_width(chanzi_btn, 2, LV_PART_MAIN);
		lv_obj_set_style_border_color(chanzi_btn, lv_color_hex(0xffffff), LV_PART_MAIN);
        ...
        for(i=0;i<5;i++)
            {
                ...
                little_car[i].y=i;
                little_car[i].zidan=lv_img_create(map1);
                lv_obj_set_pos(little_car[i].zidan,little_car[i].x,i*82+97);
                lv_img_set_src(little_car[i].zidan,&little_car_img);
                lv_img_set_zoom(little_car[i].zidan,384);
                lv_obj_set_user_data(little_car[i].zidan,&little_car[i]);
...
{
	lv_obj_t * cherry=(lv_obj_t *)var;
	lv_img_set_zoom(cherry,255+v*8);
}
...
static void cherry_delete_cb(lv_anim_t * a)
{
		...
		for(i=0;i<max_zb_count;i++)
	    {if(zb_matrix[i].blood>0&&lv_obj_get_x(zb_matrix[i].zb)-x<120&&x-lv_obj_get_x(zb_matrix[i].zb)<120&&lv_obj_get_y(zb_matrix[i].zb)-y<120&&y-lv_obj_get_y(zb_matrix[i].zb)<120)
				 {
					zb_matrix[i].blood=0;
					zb_matrix[i].zb_class->zb_del_cb(&zb_matrix[i]);
				 }
...
static void lajiao_delete_cb(lv_anim_t * a)
{
	  int i;
		...
		int y=(lv_obj_get_y(lajiao)-66)/81;

		lv_obj_t * huoyan=lv_img_create(map1);
		lv_img_set_src(huoyan, &huoyan_img);
	  lv_obj_set_y(huoyan,y*81+90);
    lv_obj_set_size(huoyan,720,45);
...
static void plant_anim_cb(void * var, int32_t v)
{
  ...
  lv_img_set_angle(xxx->plant,-v*10);
	...
	if(v>=0)
  {lv_obj_set_y(xxx->plant,xxx->y*81+76+1.5*v/2);}
	else
	{
		lv_obj_set_y(xxx->plant,xxx->y*81+76-1.5*v/2);
	}
			...
			for(j=0;j<max_zb_count;j++)
			  {
					if(zb_matrix[j].alive==0)
					{
						...
						zb_matrix[j].y=rand()%5;
						zb_matrix[j].x=720;
						lv_obj_set_pos(zb_matrix[j].zb,720,zb_matrix[j].y*81+90);
						lv_img_set_pivot(zb_matrix[j].zb, 15, 59);
...
static void jumpzb_start_anim_cb(zb_type * xzb)
{
		...
		lv_anim_set_path_cb(&a, lv_anim_path_ease_in);
		lv_anim_set_time(&a,300);
		lv_anim_set_values(&a,xzb->y*81+52,xzb->y*81+8);
		lv_anim_set_playback_time(&a, 500);
		lv_anim_set_repeat_count(&a, LV_ANIM_REPEAT_INFINITE);
		lv_anim_start(&a);
}
...
static void carzb_start_anim_cb(zb_type * xzb)
{
		...
		lv_anim_set_exec_cb(&a,jumpzb_jump_cb);
		lv_anim_set_time(&a,100);
		lv_anim_set_values(&a,xzb->y*81+30,xzb->y*81+33);
		lv_anim_set_playback_time(&a,100);
		lv_anim_set_repeat_count(&a, LV_ANIM_REPEAT_INFINITE);
		lv_anim_start(&a);
}
...
static void qiqiuzb_start_anim_cb(zb_type * xzb)
{
		...
		lv_anim_set_exec_cb(&a,jumpzb_jump_cb);
		lv_anim_set_time(&a,1000);
		lv_anim_set_values(&a,xzb->y*81,xzb->y*81+30);
        lv_anim_set_path_cb(&a, lv_anim_path_ease_in_out);
		lv_anim_set_playback_time(&a,1000);
		lv_anim_set_repeat_count(&a, LV_ANIM_REPEAT_INFINITE);
		lv_anim_start(&a);
			...
			for(j=0;j<max_quantity;j++)
			  {
					if(shine[j].alive==0)
					{
                        ...
                        lv_img_set_src(shine[j].shine, &sunshine);
                        lv_obj_add_flag(shine[j].shine, LV_OBJ_FLAG_CLICKABLE);
                        shine[j].x=rand()%600+45;
                        shine[j].y=rand()%330+75;
                        lv_obj_set_pos(shine[j].shine,shine[j].x,shine[j].y);
                        lv_obj_add_event_cb(shine[j].shine,shine_del_cb1,LV_EVENT_RELEASED,&shine[j]);
                        ...
                        lv_anim_set_path_cb(&a1, lv_anim_path_ease_out);
                        lv_anim_set_time(&a1,1000);
                        lv_anim_set_values(&a1,shine[j].y*1.5,shine[j].y*1.5+30);
                        lv_anim_set_user_data(&a1,&shine[j]);
                        lv_anim_start(&a1);
                        return;
					}
...
static void add_zidan_cb(lv_timer_t * t)
{
	...
	for(i=0;i<max_quantity;i++)
	{
		if(zidan[i].alive==0)
		{
			...
			zidan[i].zidan=lv_btn_create(map1);
			lv_obj_set_pos(zidan[i].zidan,zidan[i].x,y*81+79);
			lv_obj_set_size(zidan[i].zidan,16,16);
			lv_obj_set_style_bg_color(zidan[i].zidan,lv_color_hex(0x40ff40), LV_PART_MAIN);
			lv_obj_set_style_shadow_color(zidan[i].zidan,lv_color_hex(0x004000),LV_PART_MAIN);
			lv_obj_set_style_radius(zidan[i].zidan,8,LV_PART_MAIN);
...
static void add_shandian_cb(lv_timer_t * t)
{
	...
	plant_type * shandian=(plant_type *)t->user_data;
	x1=lv_obj_get_x(shandian->plant)+45;
	y1=lv_obj_get_y(shandian->plant)+15;
        ...
        if(zb_matrix[j].blood==0)
            {
              zb_matrix[j].zb_class->zb_del_cb(&zb_matrix[j]);
                ...
                x2=lv_obj_get_x(zb_matrix[j].zb)+45;
                y2=lv_obj_get_y(zb_matrix[j].zb)+30;
                                     ...
                                     if((zb_matrix[j].x-zidan[i].x<15)&&(zidan[i].x-zb_matrix[j].x<10))
                                          {
                                            ...
                                            lv_obj_t * zidan_split=lv_img_create(map1);
                                            lv_img_set_src(zidan_split, &zidan_split_img);
                                            lv_obj_set_pos(zidan_split,lv_obj_get_x(zidan[i].zidan)-12,lv_obj_get_y(zidan[i].zidan)-12);
                                            lv_obj_del_delayed(zidan_split,500);
                                            lv_obj_del(zidan[i].zidan);
                            ...
                            for(i=0;i<max_zb_count;i++)
                             {
                                if(zb_matrix[i].blood>0)
                                {
                                    if((zb_matrix[i].y==wogua->y)&&(zb_matrix[i].x-wogua->x*74<75)&&(zb_matrix[i].x-wogua->x*74>75))
                                     {
                                        wogua->blood=0;
                                        zb_matrix[i].blood=0;
                            ...
                            for(i=0;i<max_zb_count;i++)
                             {
                                if(zb_matrix[i].blood>0&&(zb_matrix[i].zb_class->zb_kind==Normalzb||zb_matrix[i].zb_class->zb_kind==carzb))
                                {
                                    if((zb_matrix[i].y==dici->y)&&(zb_matrix[i].x-dici->x*74<75)&&(zb_matrix[i].x-dici->x*74>-75))
                                     {
...
static void wogua_delect_zb_cb(lv_anim_t * a)
{
    ...
    lv_obj_t * user=(lv_obj_t *)a->var;
    lv_img_set_src(user,&wogua_dead_img);
    lv_obj_set_y(user,lv_obj_get_y(user)+40);
}
...
for(j=0;j<5;j++)
        ...
        if(little_car[j].alive==1)
         {
            for(i=0;i<max_zb_count;i++)
            {
                if(zb_matrix[i].blood>0&&zb_matrix[i].y==j&&zb_matrix[i].x<30)
                {
                    little_car[j].alive=0;
                    lv_anim_t a2;
                    lv_anim_init(&a2);
    ...
    for(i=0;i<max_zb_count;i++)
            {
                if(zb_matrix[i].blood>0&&zb_matrix[i].x<-15)
                {
                    game_over_cb();
                    return;
                }
...
static void shine_del_cb1(lv_event_t * e)
{
  ...
  lv_anim_set_var(&a1,user->shine);
	...
	lv_anim_set_path_cb(&a1, lv_anim_path_ease_out);
	lv_anim_set_time(&a1,500);
	lv_anim_set_values(&a1,user->x,15);
	lv_anim_start(&a1);
...
static void map_click_cb(lv_event_t * e)
{
	...
	x=(click_point.x-26)/75;
	y=(click_point.y-80)/80;
...
static void plant_del_cb(lv_event_t * e)
{
    plant_type * plant=(plant_type *)lv_event_get_user_data(e);


    if(chanzi_btn_select)
    {
        #if(plant_stackable!=1)
        map_flag[plant->y][plant->x]=0;
        #endif
        plant->alive=0;
        plant->x=0;
        plant->y=0;
        lv_anim_del(plant,plant_anim_cb);
        lv_timer_del(plant->timer);
        lv_obj_del(plant->plant);
        chanzi_btn_select=0;
        lv_obj_set_style_border_opa(chanzi_btn,0,LV_PART_MAIN);
    }
...
static void jiguangdou_del_cb(lv_event_t * e)
{
    plant_type * plant=(plant_type *)lv_event_get_user_data(e);
    ...
    if(chanzi_btn_select)
    {
        ...
        plant->x=0;
        plant->y=0;
        lv_anim_del(plant,jiguangdou_anim_cb);
        lv_timer_del(plant->timer);
        lv_obj_del(plant->plant);
        chanzi_btn_select=0;
        lv_obj_set_style_border_opa(chanzi_btn,0,LV_PART_MAIN);
					...
					if(sunflower[j].alive==0)
							{
								...
								sunflower[j].plant=lv_img_create(map1);
								lv_img_set_src(sunflower[j].plant, &sun_img);
								lv_obj_set_pos(sunflower[j].plant,x*77+40,y*81+76);
								lv_img_set_pivot(sunflower[j].plant, 23, 55);

								lv_obj_add_flag(sunflower[j].plant,LV_OBJ_FLAG_CLICKABLE);
								lv_obj_add_event_cb(sunflower[j].plant,plant_del_cb,LV_EVENT_RELEASED,&sunflower[j]);
					...
					if(wandouflower[j].alive==0)
							{
								...
								wandouflower[j].plant=lv_img_create(map1);
								lv_img_set_src(wandouflower[j].plant, &wandou_img);
								lv_obj_set_pos(wandouflower[j].plant,x*77+30,y*81+76);
								lv_img_set_pivot(wandouflower[j].plant, 27, 50);
                                lv_obj_add_flag(wandouflower[j].plant,LV_OBJ_FLAG_CLICKABLE);
								lv_obj_add_event_cb(wandouflower[j].plant,plant_del_cb,LV_EVENT_RELEASED,&wandouflower[j]);
					...
					if(shandian[j].alive==0)
							{
								...
								shandian[j].plant=lv_img_create(map1);
								lv_img_set_src(shandian[j].plant, &shandian_img);
								lv_obj_set_pos(shandian[j].plant,x*77+30,y*81+76);
								lv_img_set_pivot(shandian[j].plant, 27, 50);
                                lv_obj_add_flag(shandian[j].plant,LV_OBJ_FLAG_CLICKABLE);
								lv_obj_add_event_cb(shandian[j].plant,plant_del_cb,LV_EVENT_RELEASED,&shandian[j]);
					...
					if(jiguangdou[j].alive==0)
							{
								...
								jiguangdou[j].plant=lv_img_create(map1);
								lv_img_set_src(jiguangdou[j].plant, &jiguangdou_img);
								lv_obj_set_pos(jiguangdou[j].plant,x*77+40,y*81+65);
								lv_img_set_pivot(jiguangdou[j].plant, 27, 50);
                                lv_obj_add_flag(jiguangdou[j].plant,LV_OBJ_FLAG_CLICKABLE);
								lv_obj_add_event_cb(jiguangdou[j].plant,jiguangdou_del_cb,LV_EVENT_RELEASED,&jiguangdou[j]);
...
static void add_jiguang_cb(lv_timer_t * t)
{
		...
		jiguang_point[point_count][0].x=x*74+102;
		jiguang_point[point_count][0].y=y*81+96;
		jiguang_point[point_count][1].x=lv_disp_get_hor_res(lv_disp_get_default());
		jiguang_point[point_count][1].y=y*81+96;

		lv_obj_t * jiguangkou=lv_img_create(map1);
		lv_obj_set_pos(jiguangkou,jiguang_point[point_count][0].x-22,jiguang_point[point_count][0].y-10);
		lv_img_set_src(jiguangkou,&jiguangkou_img);
		lv_img_set_zoom(jiguangkou,512);
		lv_obj_move_foreground(jiguangkou);
		lv_obj_del_delayed(jiguangkou,500);
					...
					if(wogua[j].alive==0)
							{
								...
								lv_img_set_src(wogua[j].plant, &wogua_img);
								lv_obj_clear_flag(wogua[j].plant, LV_OBJ_FLAG_CLICKABLE);
								lv_obj_set_pos(wogua[j].plant,x*77+35,y*81+76);
								lv_obj_set_user_data(wogua[j].plant,&wogua[j]);
								wogua[j].timer=lv_timer_create(timer_wogua_test_cb,200,&wogua[j]);

                                lv_obj_add_flag(wogua[j].plant,LV_OBJ_FLAG_CLICKABLE);
								lv_obj_add_event_cb(wogua[j].plant,plant_del_cb,LV_EVENT_RELEASED,&wogua[j]);
					...
					if(dici[j].alive==0)
							{
								...
								lv_img_set_src(dici[j].plant, &dici_img);
								lv_obj_clear_flag(dici[j].plant, LV_OBJ_FLAG_CLICKABLE);
								lv_obj_set_pos(dici[j].plant,x*77+27,y*81+111);
								lv_obj_move_background(dici[j].plant);
								lv_obj_set_user_data(dici[j].plant,&dici[j]);
								dici[j].timer=lv_timer_create(timer_dici_test_cb,500,&dici[j]);
								lv_obj_add_flag(dici[j].plant,LV_OBJ_FLAG_CLICKABLE);
								lv_obj_add_event_cb(dici[j].plant,plant_del_cb,LV_EVENT_RELEASED,&dici[j]);
...
static void lajiao_creat_cb(card_btn_type * card_btn,int x,int y)
{
		lv_obj_t * lajiao=lv_img_create(map1);
		lv_img_set_src(lajiao, &lajiao_img);
		lv_obj_set_pos(lajiao,x*77+30,y*81+76);
...
static void cherry_creat_cb(card_btn_type * card_btn,int x,int y)
{
			lv_obj_t * cherry=lv_img_create(map1);
			lv_img_set_src(cherry, &cherry_img);
			lv_obj_set_pos(cherry,x*77+30,y*81+76);
...
static void hedan_creat_cb(card_btn_type * card_btn,int x,int y)
{
		...
		lv_img_set_src(nuclear, &nuclear_img);
		lv_obj_clear_flag(nuclear, LV_OBJ_FLAG_CLICKABLE);
		lv_obj_set_pos(nuclear,333,-150);
